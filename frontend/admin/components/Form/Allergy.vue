...
export default class AllergyForm extends Vue {
  @Prop() editData!: any;
  ...
  async submitForm() {
    if (this.editData) {
      await this.$axios.put("/admin/allergies/" + this.editData._id, this.data);
    } else {
      await this.$axios.post("/admin/allergies/", this.data);
    }
    this.onClose();
  }
}
