...
export class OrderController {
  ...
  async menu(@Request() req: any, @Param('id') id: string) {
    if (!mongoose.Types.ObjectId.isValid(id)) {
      throw new HttpException('正しいIDを指定してください', 403);
    }
    const table = await this.tableModel.findById(id);
    if (!table) {
      throw new HttpException('テーブルが見つかりませんでした', 403);
    }
    const menu = await this.menuModel.find();
    if (!menu) {
      throw new HttpException('メニューがありません', 403);
    }
    const category = await this.categoryModel
      .aggregate()
      .project({ name: '$_id', label: '$title' })
      .exec();
  ...
  async confirm(
    ...
    if (!this.tableModel.findById(tableId)) {
      throw new HttpException('テーブルを確認してください', 403);
    }
    if (createOrderDto.detail.length < 1) {
      throw new HttpException('注文内容を確認してください', 403);
    }
  ...
  async cancel(@Req() request: any, @Param('id') id: string) {
    ...
    if (!order) {
      throw new HttpException('注文がありません', 403);
    }
    if (order.status !== 0) {
      throw new HttpException('支払中の注文はキャンセルできません', 403);
    }
    order.status = -1;
    order.save();
    return {
  ...
  async detail(@Req() request: any, @Param('id') id: string) {
    const order = await this.orderModel
      .findOne({ _id: id, user: request.user._id })
      .populate('table')
      .populate({
        path: 'detail.menu',
        model: 'Menu',
    ...
    if (!order) {
      throw new HttpException('注文が見つかりませんでした', 403);
    }
    return {
      order: order,
    };
