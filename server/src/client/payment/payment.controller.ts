import { Controller, UseGuards, Get, Param, HttpException, NotFoundException, Query, Post, Body, Req } from '@nestjs/common';
...
export class PaymentController {
    ...
    async line_pay_confirm(@Req() request: any, @Body() paymentConfirm: PaymentConfirm) {
        const order = await this.orderModel.findOne({ _id: paymentConfirm.orderId, user: request.user._id });

        switch (order.payment) {
            case 'line-pay':
                const payment = await this.paymentService.LINE_PAY_CONFIRM(order.total, paymentConfirm.transactionId);
                const orderUpdate = await this.orderModel.findOneAndUpdate({ _id: paymentConfirm.orderId, user: request.user._id }, {
    ...
    async line_pay(@Req() request: any, @Param('id') id: string) {
        ...
        if (order.status !== 0) { throw new HttpException('注文状態が正しくありません', 403); }
        const payment = await this.paymentService.LINE_PAY_APPLY(order._id, order.total);
        const orderUpdate = await this.orderModel.findOneAndUpdate({ _id: id, user: request.user._id }, { payment: 'line-pay' });
        if (orderUpdate) {
            return {
                payment: 'line-pay',
    ...
    async cash_pay(@Req() request: any, @Param('id') id: string) {
        const order = await this.orderModel.findOne({ _id: id, user: request.user._id });
        if (order.status !== 0) { throw new HttpException('注文状態が正しくありません', 403); }
        const orderUpdate = await this.orderModel.findOneAndUpdate({ _id: id, user: request.user._id }, { payment: 'cash' });
        if (orderUpdate) {
            return {
                payment: 'cash',
                total: orderUpdate.total,
